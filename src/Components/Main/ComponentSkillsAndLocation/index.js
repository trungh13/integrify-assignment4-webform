  ...
  render() {
    ...
    const renderSkillList=skillsDesign.map(skill=>
      <React.Fragment key={skill.id}> 
        <input type="radio"  defaultChecked={skill.defaultChecked} name="skillsDesign" id={skill.id} value={skill.value} onChange={this.handleChange}/>
    ...
    const renderCheckBox=(type,typeString)=>type.map(checkBox=>
      <div className={styles.CheckBox} key={checkBox.id}>
        <input type="checkbox" name={typeString} id={checkBox.id} value={checkBox.value} onChange={this.handleChange}/>
    ...
    return (
      <div className={styles.ComponentSkillsAndLocation}>
        ...
        <Button btnName="Next" href="/SkillsAndLocation" onClick={this.handleButton} />
      </div>
    );
  }
}
